...
function pickWageFromAnything(o) {
  // Wage can appear in different shapes depending on report version.
  // Prefer explicit wage fields, then fall back to computed, then infer from pay/hours.
  const toNum = (v) => {
  ...
  const candidates = [
    ...
    // Sometimes nested:
    o?.computed?.wage,
    o?.computed?.hourly_rate,
    o?.computed?.hourly_wage,
    o?.computed?.pay_rate,
    ...
    o?.employee?.wage,
    o?.employee?.hourly_rate,
    o?.employee?.hourly_wage,
    o?.employee?.pay_rate,
  ];
  ...
  // Last resort: infer wage from pay/hours if both exist.
  const hrs = toNum(o?.regular_hours) ?? toNum(o?.computed?.regular_hours) ?? toNum(o?.hours) ?? toNum(o?.computed?.hours) ?? 0;
  const pay = toNum(o?.regular_pay) ?? toNum(o?.computed?.regular_pay) ?? toNum(o?.pay) ?? toNum(o?.computed?.pay) ?? 0;

  if (hrs > 0 && pay > 0) {
    const inferred = pay / hrs;

// scripts/fetch_silverware_week.mjs
// Pull week of POS orders from Silverware per store, write your overrides,
// AND upsert masterDashboard/{WEEK_OF}/stores/{STORE_ID}.labour.actualSales
//
// Usage:
...
const TZ_OFFSET_MIN = parseInt(
  process.env.SILVERWARE_TZ_OFFSET_MINUTES || '0',
  10
);
...
async function listOrders(base, token, startISO, endISO, page = 1) {
  try {
    ...
    return await swFetch(
      base,
      token,
      `/api/ThirdParty/GetOrders?${qs.toString()}`
    );
  }
}
...
async function getOrder(base, token, orderId) {
  try {
    return await swFetch(base, token, '/api/ThirdParty/GetOrder', {
      OrderID: orderId,
    });
  } catch {
...
function lineDiscount(line) {
  return num(line?.Discount) || num(line?.DiscountAmount) || 0;
}
...
async function collectForWeek(base, token, weekISO) {
  ...
  while (more) {
    ...
    for (const row of rows) {
      ...
      const o = orderLines(row).length
        ? row
        : await getOrder(
            base,
            token,
            row?.OrderID ?? row?.Id ?? row?.ID
          );
  ...
  return {
    ...
    food: round2(food),
    voids: round2(voids),
    promos: round2(promos),
  };
}

async function writeOverrides(locKey, weekISO, totals) {
  const ref = db.doc(
    `companies/aidan/locations/${locKey}/overrides/${weekISO}`
  );
  await ref.set(
    {
...
// ---- NEW: update masterDashboard with Actual Sales from POS ----
async function upsertMasterActualSales(locKey, weekISO, totals) {
  const ref = db.doc(`masterDashboard/${weekISO}/stores/${locKey}`);
  await ref.set(
    {
      storeId: locKey,
      weekISO,
      labour: {
        actualSales: totals.food, // POS "food" total (source of truth)
...
async function main() {
  ...
  for (const [locKey, cfg] of Object.entries(LOCS)) {
    ...
    try {
      ...
      await upsertMasterActualSales(locKey, weekISO, totals); // <- publish to master
      console.log(
        `[${locKey}] orders=${totals.orders} food=$${totals.food} voids=$${totals.voids} promos=$${totals.promos}`
      );
    } catch (e) {
